...
public class CountDownProgressView extends TextView {
    ...
    //TODO 圆边框的宽度
    private int circleFrameWidth = 4;
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null) {
            ...
            //TODO 园边框颜色属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_circleFrameColor)) {
                //有这个属性索引 获取用户设置的颜色
                circleFrameColor = typedArray.getColor(R.styleable.CountDownProgress_circleFrameColor, 0);
            } else {
                //没有这个属性索引 默认是深灰色
                circleFrameColor = typedArray.getColor(R.styleable.CountDownProgress_circleFrameColor, Color.parseColor("#A9A9A9"));
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mPaint.setStrokeWidth(circleFrameWidth);//设置空心线宽度
        mPaint.setColor(circleFrameColor);
        canvas.drawCircle(mBounds.centerX(), mBounds.centerY(), circleRadius - circleFrameWidth, mPaint);
