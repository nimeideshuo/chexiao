...
public class BitmapUtils {
     ...
     * @param imageView
     */
    public static void loadBitmap(Activity context, int resourcesId, ImageView imageView) {
        int bmSize = 0;
        Bitmap bm = null;
        if (true) {
            // 通过工具类来产生一个符合ImageView的缩略图，因为ImageView的大小是50x50，所以这里得到的缩略图也应该是一样大小的
            bm = BitmapUtils.decodeSampledBitmapFromResource(context.getResources(), resourcesId, imageView.getWidth(), imageView.getHeight());
        } else {
            // 直接加载原图
            bm = BitmapFactory.decodeResource(context.getResources(), resourcesId);
        }
